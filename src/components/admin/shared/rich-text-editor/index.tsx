...
export const RichTextEditor = ({
  ...
  const updateToolbarStates = () => {
    ...
    // Check for active headings
    const formatBlock = document.queryCommandValue("formatBlock");
    if (formatBlock) {
      const headingButton = document.querySelector(`[title*="${formatBlock}"]`);
    ...
    if (alignment) {
      const alignmentButton = document.querySelector(`[title*="${alignment}"]`);
      if (alignmentButton) alignmentButton.classList.add("active");
    }

    // Check for active blockquote
  ...
  return (
    <div className="rich-text-editor">
      ...
      <div
        ...
        className={`editor ${isFocused ? "ring-2 ring-[#3b82f6]" : ""} ${
          dragOver ? "ring-2 ring-green-400 bg-green-500/10" : ""
        }`}
      ...
      {uploading && (
        <div className="upload-overlay">
          <div className="flex items-center">
            <FaSpinner className="w-8 h-8 text-[#3b82f6] animate-spin mr-2" />
            <p className="text-white">Đang tải ảnh lên...</p>
          </div>
        </div>
      )}
    </div>
